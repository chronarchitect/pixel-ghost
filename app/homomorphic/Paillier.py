...
from . import ModularArithmetic
from . import RabinMiller
...
class PrivateKey:
    ...
    def __init__(self, p=None, q=None, n=None, lam=None, mu=None):
        if lam is not None and mu is not None:
            self.λ = lam
            self.μ = mu
        elif p is not None and q is not None and n is not None:
            self.λ = ModularArithmetic.lcm( p-1, q-1)
            self.μ = ModularArithmetic.multiplicative_inverse( self.λ, n)
        else:
            raise ValueError("Must provide either (p, q, n) or (lam, mu)")
